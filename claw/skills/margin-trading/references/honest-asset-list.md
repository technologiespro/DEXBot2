...
```bash
node claw/scripts/honest_assets_report.js --json
```

Filter by `type === 'MPA'` for margin-tradeable assets. The `honest_ecosystem.js` module's `isMpa()` function checks for `bitasset_data_id`.
