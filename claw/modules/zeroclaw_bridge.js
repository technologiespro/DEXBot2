...
function createZeroClawBridge(options = {}) {
  ...
  const runtimeName = sanitizedOptions.runtime?.name || 'zeroclaw-bridge';
...
async function runZeroClawCommand(command, options = {}) {
  const safeOptions = stripPrivateKey(options);
  if (command === 'manifest') {
    return describeZeroClawBridge(safeOptions);
  }

  const bridge = createZeroClawBridge(safeOptions);
  ...
  switch (command) {
    ...
    default:
      throw new Error(`Unsupported ZeroClaw command: ${command}`);
  }
}
