...
const { normalizeRoot, normalizeProfileDir } = require('./launcher_paths');
...
async function launcherStart(botName, options = {}) {
  ...
  const dexbotPath = path.join(ROOT, 'dexbot.js');
...
async function launcherClawOnly(options = {}) {
  const ROOT = normalizeRoot(options);
  const pm2ScriptPath = path.join(ROOT, 'pm2.js');
...
async function launcherUnlockStart(botName, options = {}) {
  const ROOT = normalizeRoot(options);
  const unlockStartPath = path.join(ROOT, 'unlock-start.js');
