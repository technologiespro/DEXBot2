...
function printLauncherHeader({ botName = null, clawOnly = false, isolated = false } = {}) {
...
function printLauncherSuccess({ botName = null, clawOnly = false, isolated = false } = {}) {
    console.log();
    console.log('='.repeat(50));
    if (clawOnly) {
...
async function runIsolated({ botName, stayResident = false }) {
    let supervisor;
    ...
    return new Promise((resolve) => {
        const interval = setInterval(async () => {
            ...
            const running = Object.values(status).some(
                (s) => s.status === 'running' || s.status === 'restarting' || s.status === 'starting'
            );
...
async function stopCredentialDaemonPid(pid) {
...
async function launchDetachedSupervisor({ botName = null, credentialDaemonPid = null } = {}) {
    ...
    const args = [path.join(ROOT, 'unlock-start.js'), '--isolated'];
    let child = null;
    if (botName) {
        args.push(botName);
    }
    ...
    try {
        ...
        if (isolated) {
            if (isDetachedSupervisorChild || forceForegroundIsolated) {
                process.exitCode = await runIsolated({ botName, stayResident: isDetachedSupervisorChild });
                return;
            }
...
function printControlStatus(status) {
